...
\emph{\footnotesize Gao Niu (Bryant University)}

In this study, we are proposing an image process algorithm to compare the before and after hurricane aerial images to estimate the house damage of the targeted area. The image process algorithm has three steps. The first step is image alignment transformation. In this process, we use roads and trees to create major grids and then transform the post-hurricane image to the same degree and angle as the pre-hurricane image. Then, we calculate the image difference. The algorithm will output an estimated percentage of damage for each identified house.
